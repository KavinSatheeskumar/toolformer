...
MAX_LEN = 1024  # Maximum wiki search length length


class WikiSearchPostprocessing(APICallPostprocessing):
    def __init__(
        self,
        ...
        self.wiki_searcher = WikiSearch
        self.api_text = "WikiSearch("
    ...
    def add_api_calls(
        ...
        **kwargs
        ...
        for j in range(len(outputs)):
            outputs[j]["WikiSearch"] = outputs[j]["generated_text"].replace(
                texts_to_test[candidate], ""
            )
            outputs[j]["Generated"] = outputs[j]["generated_text"].split("Output:")[-1]
            if "]" in outputs[j]["WikiSearch"]:
                outputs[j]["WikiSearch"] = (
                    outputs[j]["WikiSearch"].replace("WikiSearch(", "").split("]")[0]
                )
                if ")" in outputs[j]["WikiSearch"]:
                    outputs[j]["WikiSearch"] = outputs[j]["WikiSearch"].split(")")[0]
                outputs[j]["WikiSearch_text"] = (
                    "[WikiSearch(" + outputs[j]["WikiSearch"] + ")"
                )
                ...
                try:
                    outputs[j]["WikiSearch"] = self.WikiSearch(outputs[j]["WikiSearch"])
                except (requests.exceptions.RequestException):
                    continue
                if outputs[j]["WikiSearch"] is None:
                    continue
                outputs[j]["WikiSearch_output"] = [outputs[j]["WikiSearch_text"][1:], str(outputs[j]["WikiSearch"])]
                outputs[j]["WikiSearch_text"] = (
                    outputs[j]["WikiSearch_text"]
                    + "->"
                    + str(outputs[j]["WikiSearch"])
                    + "]"
                )
    ...
    def parse_article(
        ...
        for i in range((tokens.shape[1]-1)//N):
            ...
            for output in new_outputs:
                ...
                # filter by score
                if output["Score"] > 0.0:
                    outputs.append([output["Score"], output["index"]] + output["WikiSearch_output"])
        return outputs
